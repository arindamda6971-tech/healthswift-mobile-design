...
const LabSelectionScreen = () => {
  ...
  const locationState = (location.state as { patientName?: string; patientAge?: number | string; patientGender?: string; patientPhone?: string } | null) || null;
  ...
  const handleSelectLab = async (labId: string, labName: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/payment", { state: { patientName: locationState?.patientName, patientAge: locationState?.patientAge, patientGender: locationState?.patientGender, patientPhone: locationState?.patientPhone } });
      }, 500);
    } catch (error) {
